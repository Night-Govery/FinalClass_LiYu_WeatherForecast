...
#更改密码页面
@app.route('/changepassworduser/<change_user>', methods=['GET', 'POST'])
def changepassworduser(change_user):
    flash('')
    if request.method == 'POST':
        #获取密码
        password = request.form.get('password')
        change_password1 = request.form.get('change_password1')
        change_password2 = request.form.get('change_password2')
    # 如果没有登录，就返回登录页
    if "username" not in session:
        return redirect(url_for('login'))
    # 如果登录，就读取cookie并前往页面
        ...
        userID = session.get('username')
        user = User.query.filter(User.name == userID).first()
        if change_user == userID:
            if password == user.password:
                if change_password1 == change_password2:
                    try:
                        # 更改用户密码
                        user.password = change_password1
                        db.session.add(user)
                        db.session.commit()
                        flash('修改密码成功！')
                    except Exception as e:
...
#删除用户页面
@app.route('/deleteuseruser/<change_user>', methods=['GET', 'POST'])
def deleteuseruser(change_user):
    # 如果没有登录，就返回登录页
    if "username" not in session:
        return redirect(url_for('login'))
    # 如果登录，就读取cookie并前往页面
